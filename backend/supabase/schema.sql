...
create table if not exists public.registry_entries (
  ...
  created_at timestamp with time zone default timezone('utc'::text, now()) not null,
  domain text not null,
  status text check (status in ('active', 'monitored', 'inactive')) default 'monitored',
  "riskScore" integer default 0,
  "lastScanned" timestamp with time zone default timezone('utc'::text, now()),
  tags text[] default '{}'
);

alter table public.registry_entries enable row level security;

create policy "Registry entries are viewable by everyone."
  on registry_entries for select using ( true );

create policy "Authenticated users can create registry entries."
  on registry_entries for insert with check ( auth.role() = 'authenticated' );

create policy "Authenticated users can update registry entries."
  on registry_entries for update using ( auth.role() = 'authenticated' );
